    ...
    public class SimpleDeferred
    {
        public static void Main()
        {
            ...
            fap.RegisterTypeHandler(
                new AssetHandler
                {
                    ReturnedType = typeof(Fusee.Engine.Core.Scene.SceneContainer),
                    Decoder = (string id, object storage) =>
                    {
                    ...
                    DecoderAsync = async (string id, object storage) =>
                    {
                        if (Path.GetExtension(id).IndexOf("fus", System.StringComparison.OrdinalIgnoreCase) < 0)
                            throw new FileNotFoundException($"Asset {id} has no *.fus extension.");

                        var scene = await Task.Factory.StartNew(() => FusSceneConverter.ConvertFrom(Serializer.Deserialize<FusFile>((Stream)storage)));
                        return scene ?? throw new FileNotFoundException($"Asset {id} not found.");
